...
class AuthorCreateTests(TestCase):
    def test_can_create_an_author(self):
        response = self.client.post('/api/authors', self.author_data, 'application/json')
    ...
    def test_can_create_author_without_first_name(self):
        ...
        del data['first_name']
        response = self.client.post('/api/authors', data, 'application/json')
        self.assertEqual(201, response.status_code)
        self.assertEqual(1, Author.objects.count())
...
class AuthorGetEditTests(TestCase):
    ...
    def test_can_get_an_author(self):
        ...
        self.assertEqual(self.montgomery_data, response.json())

    def test_can_edit_an_author(self):
        data = self.montgomery_data
        data['first_name'] = 'Lucy'
        response = self.client.patch(f'/api/authors/{self.author.pk}', data, 'application/json')
        self.assertEqual(200, response.status_code)
        self.assertEqual('Lucy', response.json()['first_name'])
